...
$query = mysqli_query($conn, "SELECT BE.pagos, CAST(PC.fec_pag AS DATE) as fecha, CAST(PC.fec_pag AS TIME) as hora, PC.rut, BE.cor_benef, PC.nom_plan, BE.nombres, BE.apellido1, BE.apellido2, BE.fec_nac FROM pagos_clientes PC INNER JOIN beneficiarios BE on PC.pagos = BE.pagos WHERE ((CAST(PC.fec_pag AS DATE) >= '$bdesde') AND (CAST(PC.fec_pag AS DATE) <= '$bhasta') AND (PC.estado = 'S') AND (PC.clase = '$class')) ORDER BY BE.pagos, BE.cor_benef ASC");

if($query->num_rows > 0){
    $delimiter = ";";
    $filename = "REST911_benef_desde_".$bdesde. "_hasta_".$bhasta.".csv";
