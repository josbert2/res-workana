...
$query = mysqli_query($conn, "SELECT p.pagos, CAST(p.fec_pag AS DATE) as fecha, CAST(p.fec_pag AS TIME) as hora, p.t_nombres, p.t_apellido1, p.t_apellido2, p.rut, p.t_fec_nac, p.email, p.telefono, p.telefono2, p.direccion, p.comuna, p.cod_plan, p.nom_plan, p.num_benef, p.valor_plan_benef, p.cod_pago, p.tipo_pago, p.num_mes, p.cant_descuento, p.valor_plan,
CASE 
...
ELSE 'No Especifica'
END) AS responsecode
FROM pagos_clientes p WHERE ((CAST(FEC_PAG AS DATE) >= '$bdesde') AND (CAST(FEC_PAG AS DATE) <= '$bhasta') AND (IF('T' != '$class', clase = '$class', clase IS NOT NULL)) ) ORDER BY fec_pag ASC");
