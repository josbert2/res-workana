...
<!-- End Google Tag Manager (noscript) -->
    ...
    <section class="formulario">
        ...
        <div class="accordion" id="accordionExample">
            <?php
            //Paginador
            $sql_register = mysqli_query($conn, "SELECT COUNT(*) as total_registro FROM pagos_clientes WHERE ((CAST(FEC_PAG AS DATE) >= '$bdesde') AND (CAST(FEC_PAG AS DATE) <= '$bhasta') AND (IF('T' != '$class', clase = '$class', clase IS NOT NULL)) ".$crol." ) ");
            /*if(!$sql_register){
                echo mysqli_error();
            }*/
            ...
            ELSE 'No Específica'
            END) AS responsecode, US.usuario FROM pagos_clientes p INNER JOIN user US on p.userid = US.userid WHERE ((CAST(FEC_PAG AS DATE) >= '$bdesde') AND (CAST(FEC_PAG AS DATE) <= '$bhasta') AND (IF('T' != '$class', clase = '$class', clase IS NOT NULL)) ".$crol." ) order BY FEC_PAG DESC LIMIT $desde, $por_pagina");
