...
<!-- End Google Tag Manager (noscript) -->
    ...
    <section class="formulario">
        ...
        <a href="exportar_pch.php?class=<?php echo $class ?>" class="btn btn-success" target="_blank">Exportar Transacciones de hoy</a>
        ...
        <form action="buscar.php" method="get" class="form_search">
            ...
            <input type="date" step="1" min="2020-04-01" max="2020-09-04" value="2020-09-04" name="bdesde" id="bdesde" placeholder="Desde">
            </div>
            <div class="inputsss">
            <label for="hasta"> Hasta: </label>
            <input type="date" step="1" min="2020-04-01" max="2020-09-05" value="2020-09-05" name="bhasta" id="bhasta" placeholder="Hasta">
            </div>
        ...
        <div class="accordion" id="accordionExample">
            <?php
            //Paginador
            $sql_register = mysqli_query($conn, "SELECT COUNT(*) as total_registro FROM pagos_clientes WHERE ((CAST(FEC_PAG AS DATE) = '$fechahoy') AND (clase = '$class')) ");
            ...
            $query_2 = mysqli_query($conn, "SELECT p.pagos, p.fec_pag, CAST(FEC_PAG AS TIME) as hora, p.t_nombres, p.t_apellido1, p.t_apellido2, p.rut, p.t_fec_nac, p.email, p.telefono, p.telefono2, p.direccion, p.comuna, p.cod_plan, p.nom_plan, p.num_benef, p.valor_plan_benef, p.cod_pago, p.tipo_pago, p.cant_descuento, p.valor_plan, CASE 
            ...
            END AS responsecode FROM pagos_clientes p WHERE ((CAST(FEC_PAG AS DATE) = '$fechahoy') AND (p.clase = '$class')) order BY FEC_PAG DESC LIMIT $desde, $por_pagina");
