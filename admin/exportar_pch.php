...
$query = mysqli_query($conn, "SELECT p.pagos, CAST(p.fec_pag AS DATE) as fecha, CAST(p.fec_pag AS TIME) as hora, p.fec_pag, p.t_nombres, p.t_apellido1, p.t_apellido2, p.rut, p.t_fec_nac, p.email, p.telefono, p.telefono2, p.direccion, p.comuna, p.cod_plan, p.nom_plan, p.num_benef, p.valor_plan_benef, p.cod_pago, p.tipo_pago, p.num_mes, p.cant_descuento, p.valor_plan, p.campania,
CASE 
...
FROM pagos_clientes p INNER JOIN user US on p.userid = US.userid WHERE ((CAST(FEC_PAG AS DATE) = '$date') AND (IF('T' != '$class', clase = '$class', clase IS NOT NULL)) ".$crol." ) ORDER BY pagos ASC");
