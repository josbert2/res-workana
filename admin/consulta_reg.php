...
function getTransc($transc,$bdesde,$bhasta){
    ...
    if($transc == "A"){
        ...
        $sql  = "(SELECT t_cod_tran, PC.pagos, PC.clase, PC.rut, t_fec_fac, t_fecha_tbk, t_val_plan, t_buyorder, t_authorizationcode, t_payment_type_code, US.usuario, t_observa FROM pagos_clientes PC INNER JOIN pagos_transac on PC.pagos = t_cod_pagos INNER JOIN user US on t_userid = US.userid WHERE ( (CAST(t_fecha_tbk AS DATE) >= '$bdesde') AND (CAST(t_fecha_tbk AS DATE) <= '$bhasta') AND (PC.estado = 'S') ".$busq2." AND (t_estado != 'X') ) )
        UNION
        (SELECT f_cod_fall, PC.pagos, PC.clase, PC.rut, f_fec_fac, t_fecha_tbk, f_val_plan, f_buyorder, NULL, f_payment_type_code, US.usuario, f_log FROM pagos_clientes PC INNER JOIN fallidas on PC.pagos = f_cod_pagos INNER JOIN user US on f_userid = US.userid WHERE ( (CAST(t_fecha_tbk AS DATE) >= '$bdesde') AND (CAST(t_fecha_tbk AS DATE) <= '$bhasta') AND ( (PC.estado = 'S') OR (PC.estado = 'I') ) AND (f_estado = 'S') ) ) ORDER BY clase,t_fecha_tbk ASC";
