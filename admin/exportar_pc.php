...
FROM pagos_clientes p INNER JOIN user US on p.userid = US.userid WHERE ((CAST(FEC_PAG AS DATE) >= '$bdesde') AND (CAST(FEC_PAG AS DATE) <= '$bhasta') AND (IF('T' != '$class', clase = '$class', clase IS NOT NULL)) ".$crol." ) ORDER BY fec_pag ASC");
